#!/usr/bin/env bash
# loop.sh
# Outer coordinator loop: while not stopped; tick; sleep COORDINATOR_INTERVAL_SECONDS.
# Stop file is `.daft/workspace/coordinator.stop` — touch it (or send SIGTERM) to
...
function main() {
  ...
  validate_args "${@:-}"
  setup_signals
  log '🚀 coordinator loop start'
  loop
  log '✅ coordinator loop exit'
}
...
function loop() {
  while ! should_stop; do
    ...
    sleep "${COORDINATOR_INTERVAL_SECONDS}" || true
  done
  rm -f "${STOP_FILE}"
}
